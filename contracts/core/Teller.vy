...
event TellerRebalance:
    user: indexed(address)
    caller: indexed(address)
    depositAsset: indexed(address)
...
def _deposit(
    ...
    vaultAddr, vaultId = staticcall TellerUtils(utils).getVaultAddrAndId(_asset, _vaultAddr, _vaultId, _a.vaultBook, _a.missionControl)

    # one more validation -- TellerUtils repeats this check, but putting here as backup
    assert staticcall AddressRegistry(_a.vaultBook).isValidAddr(vaultAddr) # dev: invalid vault
...
def _withdraw(
    ...
    amount: uint256 = staticcall TellerUtils(utils).validateOnWithdrawal(_asset, _amount, _user, vaultAddr, vaultId, _caller, config, _a)
...
def rebalance(
    ...
    assert not deptBasics.isPaused # dev: contract paused
    assert _depositAmount != 0 # dev: cannot deposit 0
    assert _withdrawAmount != 0 # dev: cannot withdraw 0

    a: addys.Addys = addys._getAddys()

    # deposit first
    depositedAmount: uint256 = self._deposit(
        _depositAsset,
        _depositAmount,
        _user,
        empty(address),
    ...
    # withdraw second
    withdrawnAmount: uint256 = self._withdraw(
        _withdrawAsset,
        _withdrawAmount,
        _user,
        empty(address),
    ...
    # perform house keeping !!!
    # this ensures user's debt position is still healthy after the rebalance
    self._performHousekeeping(True, _user, True, a)
...
# VaultMigrator owns all validation, batching, sequencing, lifecycle state and postconditions.
# Teller exposes only exact-caller identity steps because the vault and Ledger permissions still
# require Teller's identity. Splitting source and destination steps lets VaultMigrator validate
# exact receipt and depletion before it authorizes the corresponding import or deposit.


@view
@internal
def _assertVaultMigrator():
    assert msg.sender == addys._getVaultMigratorAddr() # dev: only vault migrator allowed


@nonreentrant
@external
def executeRipeGovSource(
    _user: address,
    _asset: address,
    _sourceVault: address,
    _targetVault: address,
    _legacyMigration: RipeGovMigrationData,
    _shouldUseVaultWithdrawal: bool,
) -> RipeGovMigrationData:
    a: addys.Addys = addys._getAddys()
    self._assertVaultMigrator()

    migration: RipeGovMigrationData = _legacyMigration

    if _shouldUseVaultWithdrawal:
        ...
        migration = extcall RipeGovVault(_sourceVault).exportPositionForMigration(_user, _asset, _targetVault, a)
...
def executeRipeGovImport(
    ...
    targetShares: uint256 = extcall RipeGovVault(_targetVault).importPositionForMigration(
        _user,
        _asset,
        _sourceVault,
        _migration,
...
def executeVaultWithdrawal(
    ...
    amount: uint256 = 0
    isDepleted: bool = False
    amount, isDepleted = extcall Vault(_sourceVault).withdrawTokensFromVault(_user, _asset, max_value(uint256), self, a)
    return amount, isDepleted


@nonreentrant
@external
def executeVaultDeposit(
    _user: address,
    _asset: address,
    _amount: uint256,
    _targetVaultId: uint256,
...
def setUserConfig(
    ...
    # validate underscore wallet
    if _user != msg.sender:
        assert staticcall TellerUtils(addys._getTellerUtilsAddr()).isUnderscoreWalletOwner(_user, msg.sender, mc) # dev: not owner of underscore wallet
...
def setUndyLegoAccess(_legoAddr: address) -> bool:
    ...
    if not staticcall TellerUtils(addys._getTellerUtilsAddr()).isUnderscoreWalletOrVault(msg.sender, mc):
        return False
...
def _performHousekeeping(
    ...
    if staticcall MissionControl(_a.missionControl).shouldCheckLastTouch():
        isUndyCaller: bool = staticcall TellerUtils(addys._getTellerUtilsAddr()).isUnderscoreWalletOrVault(_user, _a.missionControl)
