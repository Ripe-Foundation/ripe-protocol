...
def addAsset(
    ...
    assert _stakersPointsAlloc == 0 and _voterPointsAlloc == 0 # dev: new asset allocations must be zero
...
def _areValidAuctionParams(_params: cs.AuctionParams) -> bool:
    if not _params.hasParams:
        return False
    if _params.startDiscount > HUNDRED_PERCENT or _params.maxDiscount >= HUNDRED_PERCENT:
        return False
...
def setAssetDebtTerms(
    ...
    self._assertDebtTermsAllowedForAccrual(_asset, _ltv, mc)
...
def _setPendingAssetConfig(
    ...
    if _actionType == ActionType.ASSET_LIQ_CONFIG:
...
@view
@internal
def _assertDebtTermsAllowedForAccrual(_asset: address, _newLtv: uint256, _missionControl: address):
    rewardVaultId: uint256 = staticcall MissionControl(_missionControl).rewardVaultId(_asset)
    if rewardVaultId == 0:
        return
    if staticcall MissionControl(_missionControl).accrualStartBlock(_asset, rewardVaultId) == max_value(uint256):
        assert _newLtv == 0 # dev: armed promotional ltv must remain zero


@internal
def _writeAssetConfig(_asset: address, _config: cs.AssetConfig, _missionControl: address):
    assert self._isValidAssetConfig(_asset, _config, _missionControl) # dev: invalid asset
    extcall MissionControl(_missionControl).setAssetConfig(_asset, _config)
...
def executePendingAction(_aid: uint256) -> bool:
    ...
    assert (
        actionType == ActionType.ASSET_ADD_NEW
        or actionType == ActionType.ASSET_LIQ_CONFIG
        or actionType == ActionType.ASSET_DEBT_TERMS
        or actionType == ActionType.ASSET_WHITELIST
    ) # dev: invalid action
    ...
    if actionType == ActionType.ASSET_ADD_NEW:
        ...
        assert p.config.stakersPointsAlloc == 0 and p.config.voterPointsAlloc == 0 # dev: new asset allocations must be zero
        self._writeAssetConfig(p.asset, p.config, mc)
        ...
        self._writeAssetConfig(p.asset, config, mc)
        ...
        self._assertDebtTermsAllowedForAccrual(p.asset, pendingTerms.ltv, mc)
