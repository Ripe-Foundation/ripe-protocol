...
class Migration:
    ...
    def _record_reconciled_transaction(self):
    ...
    def _run(self, contract_name, transaction, *args, **kwargs):
        ...
        if not tx:
            ...
            # Deployment objects stringify to verbose representations, and a
            # VyperBlueprint string contains only a Python memory address.
            # Persist the actual chain address so a restart can authenticate
            # the journal against the pending manifest.
            journal_entry = str(tx.address) if contract_name != "" else tx
            self._transactions.append(journal_entry)
            gas = 0
            if contract_name != "":
            ...
            self._last_resumed_transaction_raw = str(tx)
            self._last_resumed_transaction = _deployment_address_from_log(str(tx))
            if contract_name != "":
                self._count += 1
