...
# The source catalogue includes the inactive-candidate setup variant. This does
# not authorize that variant in the independently approved deployed inventory.
...
def static_switchboard_errors(root: Path, manifest: dict[str, Any]) -> list[str]:
    ...
    for name in FOXTROT_SOURCES:
        path = root / "contracts" / "config" / f"{name}.vy"
        if not path.is_file():
            continue  # The exact source-inventory comparison already reports it.
        foxtrot = path.read_text()
        for method in FOXTROT_SEMANTIC_METHODS:
            if f"def {method}(" not in foxtrot:
                errors.append(f"{name} semantic method missing: {method}")
        engine_calls = set(
            re.findall(
        ...
        if engine_calls != set(ENGINE_MUTATORS):
            errors.append(
                f"{name} Engine mutator calls changed: "
        ...
        if vesting_calls != set(VESTING_MUTATORS):
            errors.append(
                f"{name} Vesting mutator calls changed: "
