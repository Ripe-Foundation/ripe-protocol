...
class Network:
    ...
    launch_defaults: str
    cadence_note: str
    # address -> constant name. This also bypasses token metadata reads.
    name_overrides: Mapping[str, str] = field(default_factory=dict)
    # address -> comment label.
...
# Keep the established Base names stable. Unnamed assets without an explicit
# override are resolved through symbol() at the pinned snapshot block.
BASE_ASSET_NAMES = {
...
#
# Every policy and asset value below was read off the live {display_name}
...
def _address_label(
    ...
    """Return an existing label or the deterministic address fallback."""
    override = label_overrides.get(address.lower())
    return override or manifest_name or f"ADDRESS_{address.lower().removeprefix('0x')}"


def _safe_constant_name(value: str) -> str:
    """Turn untrusted token metadata into a valid, bounded Vyper identifier."""
    text = str(value)
    # Preserve useful camel-case word boundaries before uppercasing.
    text = re.sub(r"(?<=[a-z0-9])(?=[A-Z])", "_", text)
    text = "".join(
        ch if ch.isascii() and ch.isalnum() else "_" for ch in text
    )
    identifier = re.sub(r"_+", "_", text).strip("_").upper()
    if not identifier:
        identifier = "ASSET"
    if not identifier[0].isalpha():
        identifier = f"ASSET_{identifier}"
    return identifier[:80].rstrip("_")


def _safe_asset_comment(value: str, fallback: str) -> str:
    """Render token metadata on one printable ASCII comment line."""
    text = "".join(
        ch if ch.isascii() and ch.isprintable() else " " for ch in str(value)
    )
...
def _decode_token_symbol(raw: bytes, address: str) -> str:
    """Decode standard string and legacy bytes32 ERC-20 symbol responses."""
    data = bytes(raw)
    try:
        if len(data) == 32:
            symbol_bytes = data.rstrip(b"\x00")
        else:
            if len(data) < 64 or int.from_bytes(data[:32], "big") != 32:
                raise ValueError
            length = int.from_bytes(data[32:64], "big")
            if length == 0 or length > 256 or len(data) < 64 + length:
                raise ValueError
            symbol_bytes = data[64:64 + length]
        symbol = symbol_bytes.decode("utf-8").strip().strip("\x00")
    except (UnicodeDecodeError, ValueError):
        raise SnapshotError(
            f"token symbol returned invalid ABI data at {address}"
        ) from None
    if not symbol:
        raise SnapshotError(f"token symbol is empty at {address}")
    return symbol


def _read_token_symbol(w3, address: str, block_number: int) -> str:
    """Read symbol() at the same historical block as all configuration state."""
    from web3 import Web3

    checksum = Web3.to_checksum_address(address)
    try:
        raw = w3.eth.call(
            {"to": checksum, "data": SYMBOL_SELECTOR},
            block_identifier=block_number,
        )
    except Exception:
        raise SnapshotError(
            f"token symbol unavailable at snapshot block for {checksum}; "
            "add a network name override if this asset has no symbol()"
        ) from None
    return _decode_token_symbol(raw, checksum)
...
def build(
    ...
    *,
    ...
    # Every address the file needs a name for. Live asset and dependency
    # addresses are emitted as constants.  The Contributor blueprint is the
    # one intentional override: replacement migrations can deploy the latest
    # blueprint first and bind it without changing any snapshotted policy.
    hr = call(mc, "hrConfig")
    ...
    token_addresses = {
        addr.lower()
        for addr in [*assets, bond_asset, first_liq_asset]
    ...
    for addr in wanted:
        ...
        if manifest_name in preferred_names:
            ...
            label = network.label_overrides.get(low, const)
        elif manifest_name:
            label = manifest_name
            const = _safe_constant_name(label)
        elif low in token_addresses:
            symbol = _read_token_symbol(w3, addr, snapshot.block_number)
            fallback = f"ADDRESS_{low.removeprefix('0x')}"
            label = _safe_asset_comment(symbol, fallback)
            const = _safe_constant_name(symbol)
        else:
        ...
        addr_labels[low] = _safe_asset_comment(
            label, f"ADDRESS_{low.removeprefix('0x')}"
        )
    ...
    for const, addr in consts:
        if const == "CONTRIB_TEMPLATE":
            out.append("CONTRIB_TEMPLATE: immutable(address)\n")
        else:
    ...
    for addr in assets:
        ...
        label = addr_labels.get(
            addr.lower(),
            _address_label(addr, by_addr.get(addr.lower()), network.label_overrides),
        )
        entries.append(
            f"        # {label}\n"
