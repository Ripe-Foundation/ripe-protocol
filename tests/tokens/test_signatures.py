...
def test_permit_blacklisted_or_paused(green_token, special_signer, bob, mission_control_gov, governance, signPermit):
    amount = 100 * EIGHTEEN_DECIMALS
    signature, deadline = signPermit(green_token, special_signer, bob, amount)
    green_token.setBlacklist(special_signer, True, sender=mission_control_gov.address)
    with boa.reverts():
        green_token.permit(special_signer, bob, amount, deadline, signature)
    green_token.setBlacklist(special_signer, False, sender=mission_control_gov.address)
