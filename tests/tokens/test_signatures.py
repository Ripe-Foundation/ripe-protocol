...
def test_permit_blacklisted_or_paused(green_token, special_signer, bob, switchboard_three, governance, signPermit):
    amount = 100 * EIGHTEEN_DECIMALS
    signature, deadline = signPermit(green_token, special_signer, bob, amount)
    green_token.setBlacklist(special_signer, True, sender=switchboard_three.address)
    with boa.reverts():
        green_token.permit(special_signer, bob, amount, deadline, signature)
    green_token.setBlacklist(special_signer, False, sender=switchboard_three.address)
