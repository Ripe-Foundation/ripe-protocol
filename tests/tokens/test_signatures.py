...
def test_permit_blacklisted_or_paused(green_token, special_signer, bob, control_room, governance, signPermit):
    amount = 100 * EIGHTEEN_DECIMALS
    signature, deadline = signPermit(green_token, special_signer, bob, amount)
    green_token.setBlacklist(special_signer, True, sender=control_room.address)
    with boa.reverts():
        green_token.permit(special_signer, bob, amount, deadline, signature)
    green_token.setBlacklist(special_signer, False, sender=control_room.address)
