...
def test_green_token_blacklist_functionality(green_token, whale, bob, switchboard_three, governance):
    ...
    # Test blacklist
    green_token.setBlacklist(whale, True, sender=switchboard_three.address)
    ...
    green_token.setBlacklist(bob, True, sender=switchboard_three.address)
    assert green_token.blacklisted(bob)

    green_token.setBlacklist(whale, False, sender=switchboard_three.address)
...
def test_green_token_minting_edge_cases(green_token, governance, whale, credit_engine, switchboard_three, alice):
...
def test_green_token_transfer_edge_cases(green_token, whale, bob, governance, switchboard_three):
...
def test_green_token_transfer_from_edge_cases(green_token, whale, bob, alice, governance, switchboard_three):
    ...
    green_token.setBlacklist(bob, False, sender=switchboard_three.address)
    green_token.pause(True, sender=governance.address)
    with boa.reverts("token paused"):
        green_token.transferFrom(whale, alice, transfer_amount, sender=bob)
    green_token.pause(False, sender=governance.address)
...
def test_green_token_blacklist_edge_cases(green_token, governance, switchboard_three, credit_engine):
    """Test blacklist edge cases"""
    # Test blacklisting self
    with boa.reverts("invalid blacklist recipient"):
        green_token.setBlacklist(green_token.address, True, sender=switchboard_three.address)
    
    # Test blacklisting zero address
    with boa.reverts("invalid blacklist recipient"):
        green_token.setBlacklist(ZERO_ADDRESS, True, sender=switchboard_three.address)
    ...
    green_token.setBlacklist(test_address, True, sender=switchboard_three.address)
...
def test_green_token_events(green_token, whale, bob, governance, switchboard_three, mock_ripe_hq):
    """Test token events"""
    # Test Transfer event
    transfer_amount = 100 * EIGHTEEN_DECIMALS
    assert green_token.transfer(bob, transfer_amount, sender=whale)
    ...
    assert green_token.setBlacklist(bob, True, sender=switchboard_three.address)
