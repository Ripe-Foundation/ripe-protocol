...
def test_price_source_data_pause(
    ...
    # Test pause
    price_source_mock.pause(True, sender=switchboard_alpha.address)
    ...
    # Test unpause
    price_source_mock.pause(False, sender=switchboard_alpha.address)
...
def test_price_source_data_recover_funds(
    ...
    with boa.reverts("nothing to recover"):
        price_source_mock.recoverFunds(bob, alpha_token, sender=switchboard_alpha.address)
    
    # Test recovery with invalid recipient
    with boa.reverts("invalid recipient or asset"):
        price_source_mock.recoverFunds(ZERO_ADDRESS, alpha_token, sender=switchboard_alpha.address)
    
    # Test recovery with invalid asset
    with boa.reverts("invalid recipient or asset"):
        price_source_mock.recoverFunds(bob, ZERO_ADDRESS, sender=switchboard_alpha.address)
...
def test_price_source_data_recover_funds_many(
    ...
    price_source_mock.recoverFundsMany(bob, [], sender=switchboard_alpha.address)
    # No events should be emitted for empty array
    
    # Test recovery with zero balances
    with boa.reverts("nothing to recover"):
        price_source_mock.recoverFundsMany(bob, [alpha_token, bravo_token], sender=switchboard_alpha.address)
    
    # Test recovery with invalid recipient
    with boa.reverts("invalid recipient or asset"):
        price_source_mock.recoverFundsMany(ZERO_ADDRESS, [alpha_token, bravo_token], sender=switchboard_alpha.address)
    ...
    # Test recovery with invalid asset
    with boa.reverts("invalid recipient or asset"):
        price_source_mock.recoverFundsMany(bob, [alpha_token, ZERO_ADDRESS], sender=switchboard_alpha.address)
...
def _fill_priced_assets(price_source_mock, count=50):
    tokens = []
    for i in range(count):
        token = boa.env.generate_address(f"priced-asset-{i}")
        tokens.append(token)
        price_source_mock.setPrice(token, 100 + i)
    return tokens


def test_price_source_data_rejects_51st_asset_atomically(price_source_mock):
    ...
    rejected = boa.env.generate_address("priced-asset-51")
...
def test_price_source_data_can_add_after_removing_one_at_capacity(price_source_mock):
    tokens = _fill_priced_assets(price_source_mock, 50)
    removed = tokens[7]
    price_source_mock.disablePriceFeed(removed)
    assert price_source_mock.numAssets() == 50
    assert price_source_mock.indexOfAsset(removed) == 0

    replacement = boa.env.generate_address("priced-asset-replacement")
    price_source_mock.setPrice(replacement, 321)
    assets = price_source_mock.getPricedAssets()
    assert len(assets) == 50
    assert replacement in assets
    assert removed not in assets
    assert price_source_mock.indexOfAsset(replacement) != 0
    assert price_source_mock.hasPriceFeed(replacement)

    with boa.reverts("too many assets"):
        price_source_mock.setPrice(boa.env.generate_address("priced-asset-overflow"), 1)
