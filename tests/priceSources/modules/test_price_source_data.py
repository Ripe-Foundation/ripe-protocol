...
def test_price_source_data_pause(
    ...
    # Test pause
    price_source_mock.pause(True, sender=control_room.address)
    ...
    # Test unpause
    price_source_mock.pause(False, sender=control_room.address)
...
def test_price_source_data_recover_funds(
    ...
    with boa.reverts("nothing to recover"):
        price_source_mock.recoverFunds(bob, alpha_token, sender=control_room.address)
    
    # Test recovery with invalid recipient
    with boa.reverts("invalid recipient or asset"):
        price_source_mock.recoverFunds(ZERO_ADDRESS, alpha_token, sender=control_room.address)
    
    # Test recovery with invalid asset
    with boa.reverts("invalid recipient or asset"):
        price_source_mock.recoverFunds(bob, ZERO_ADDRESS, sender=control_room.address)
...
def test_price_source_data_recover_funds_many(
    ...
    price_source_mock.recoverFundsMany(bob, [], sender=control_room.address)
    # No events should be emitted for empty array
    
    # Test recovery with zero balances
    with boa.reverts("nothing to recover"):
        price_source_mock.recoverFundsMany(bob, [alpha_token, bravo_token], sender=control_room.address)
    
    # Test recovery with invalid recipient
    with boa.reverts("invalid recipient or asset"):
        price_source_mock.recoverFundsMany(ZERO_ADDRESS, [alpha_token, bravo_token], sender=control_room.address)
    ...
    # Test recovery with invalid asset
    with boa.reverts("invalid recipient or asset"):
        price_source_mock.recoverFundsMany(bob, [alpha_token, ZERO_ADDRESS], sender=control_room.address)
    
    # Test recovery with too many assets
    too_many_assets = [alpha_token] * 21  # MAX_RECOVER_ASSETS is 20
    with boa.reverts():
        price_source_mock.recoverFundsMany(bob, too_many_assets, sender=control_room.address)
