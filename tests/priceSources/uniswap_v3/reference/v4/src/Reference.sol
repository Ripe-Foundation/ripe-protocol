...
contract Reference {
    ...
    function quote(int24 tick, uint128 amount, address base, address counter) external pure returns (uint256) {
        ...
        if (s <= type(uint128).max) {
            uint256 ratio192 = uint256(s)*s;
            return base < counter ? FullMath.mulDiv(ratio192, amount, 1<<192) : FullMath.mulDiv(1<<192, amount, ratio192);
        }
        uint256 ratio128 = FullMath.mulDiv(s,s,1<<64);
        return base < counter ? FullMath.mulDiv(ratio128,amount,1<<128) : FullMath.mulDiv(1<<128,amount,ratio128);
    }
}
