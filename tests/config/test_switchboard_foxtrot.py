...
# Auction surface lives on Foxtrot (moved off Charlie). These tests must stay
# in this file so the reserve-engine feature gate's 85% Foxtrot coverage sees
# the new block. Charlie still has overlapping cases in its own suite.


def test_foxtrot_auction_rejects_non_gov_and_bad_params(
    switchboard_foxtrot,
    governance,
    alice,
    bob,
    ...
    with boa.reverts("no perms"):
        switchboard_foxtrot.startAuction(user, 1, asset, sender=bob)
    with boa.reverts("no perms"):
        switchboard_foxtrot.startManyAuctions([(user, 1, asset)], sender=alice)
    with boa.reverts("no perms"):
        switchboard_foxtrot.pauseAuction(user, 1, asset, sender=bob)
    with boa.reverts("no perms"):
        switchboard_foxtrot.pauseManyAuctions([(user, 1, asset)], sender=alice)

    with boa.reverts("invalid parameters"):
        switchboard_foxtrot.startAuction(ZERO_ADDRESS, 1, asset, sender=governance.address)
    with boa.reverts("invalid parameters"):
        switchboard_foxtrot.startAuction(user, 1, ZERO_ADDRESS, sender=governance.address)
    with boa.reverts("no auctions provided"):
        switchboard_foxtrot.startManyAuctions([], sender=governance.address)
    with boa.reverts("invalid parameters"):
        switchboard_foxtrot.pauseAuction(ZERO_ADDRESS, 1, asset, sender=governance.address)
    with boa.reverts("invalid parameters"):
        switchboard_foxtrot.pauseAuction(user, 1, ZERO_ADDRESS, sender=governance.address)
    with boa.reverts("no auctions provided"):
        switchboard_foxtrot.pauseManyAuctions([], sender=governance.address)

    with boa.reverts("cannot start auction"):
        switchboard_foxtrot.startAuction(user, 1, asset, sender=governance.address)
    with boa.reverts("cannot start auction"):
        switchboard_foxtrot.startManyAuctions(
            [(user, 1, asset), (user, 2, asset)],
            sender=governance.address,
        )
    with boa.reverts("DynArray[FungAuctionConfig, 20] bounds check"):
        switchboard_foxtrot.startManyAuctions(
            [(user, 1, asset)] * 21,
            sender=governance.address,
        )
...
def test_foxtrot_pause_auction_queue_execute_and_cancel(
    switchboard_foxtrot,
    governance,
    alice,
    alpha_token,
    ...
    pause_id = switchboard_foxtrot.pauseAuction(
        user, 1, asset, sender=governance.address
    )
    assert switchboard_foxtrot.actionType(pause_id) == 16  # PAUSE_AUCTION
    stored = switchboard_foxtrot.pendingPauseAuctionActions(pause_id)
    assert stored.liqUser == user
    assert stored.vaultId == 1
    assert stored.asset == asset

    cancel_id = switchboard_foxtrot.pauseAuction(
        user, 2, asset, sender=governance.address
    )
    assert switchboard_foxtrot.cancelPendingAction(
        cancel_id, sender=governance.address
    )
    assert switchboard_foxtrot.actionType(cancel_id) == 0

    boa.env.time_travel(blocks=switchboard_foxtrot.actionTimeLock())
    assert switchboard_foxtrot.executePendingAction(
        pause_id, sender=governance.address
    )
    executed = filter_logs(switchboard_foxtrot, "PauseAuctionExecuted")
    assert executed[-1].liqUser == user
    assert executed[-1].vaultId == 1
    assert executed[-1].asset == asset
    assert switchboard_foxtrot.actionType(pause_id) == 0


def test_foxtrot_pause_many_auctions_queue_and_execute(
    switchboard_foxtrot,
    governance,
    alice,
    alpha_token,
    ...
    auctions = [
        (alice, 1, alpha_token.address),
        (alice, 2, alpha_token.address),
    ]
    action_id = switchboard_foxtrot.pauseManyAuctions(
        auctions, sender=governance.address
    )
    assert switchboard_foxtrot.actionType(action_id) == 32  # PAUSE_MANY_AUCTIONS
    boa.env.time_travel(blocks=switchboard_foxtrot.actionTimeLock())
    assert switchboard_foxtrot.executePendingAction(
        action_id, sender=governance.address
    )
    logs = filter_logs(switchboard_foxtrot, "PauseManyAuctionsExecuted")
    assert logs[-1].numAuctionsPaused == 0
