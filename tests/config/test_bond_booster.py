...
@pytest.fixture(scope="module")
def bond_booster_fresh(ripe_hq_deploy):
    """Deploy a fresh BondBooster contract for testing initial boosts"""
    def _bond_booster_fresh(_initialBoosts=[]):
        return boa.load(
            "contracts/config/BondBooster.vy",
            ripe_hq_deploy,
            1000 * EIGHTEEN_DECIMALS,  # _maxBoost
            100,                        # _maxUnits
            _initialBoosts,             # _initialBoosts
        )
    return _bond_booster_fresh
...
def test_bond_booster_deployment_params(bond_booster):
    """Test BondBooster deploys with correct parameters"""
    # The fixture sets MAX_BOOST to 1000 * 10^18 and MAX_UNITS to 100
    # We can't directly access immutables, but we can test their effects
    pass


def test_bond_booster_deployment_with_initial_boosts(bond_booster_fresh, valid_booster_config, alice):
    """Test BondBooster deployment with initial boosts"""
    # Create initial boosts list
    initial_boosts = [
        (
            alice,
            50 * EIGHTEEN_DECIMALS,  # ripePerUnit
            25,                       # maxUnitsAllowed
            1000000,                  # expireBlock
        ),
        (
            valid_booster_config["user"],
            valid_booster_config["ripePerUnit"],
            valid_booster_config["maxUnitsAllowed"],
            valid_booster_config["expireBlock"],
        ),
    ]
    
    # Deploy with initial boosts
    bond_booster = bond_booster_fresh(initial_boosts)
    
    # Verify initial boosts were set
    config1 = bond_booster.config(alice)
    assert config1[0] == alice  # user
    assert config1[1] == 50 * EIGHTEEN_DECIMALS  # ripePerUnit
    assert config1[2] == 25  # maxUnitsAllowed
    assert config1[3] == 1000000  # expireBlock
    
    config2 = bond_booster.config(valid_booster_config["user"])
    assert config2[0] == valid_booster_config["user"]
    assert config2[1] == valid_booster_config["ripePerUnit"]
    assert config2[2] == valid_booster_config["maxUnitsAllowed"]
    assert config2[3] == valid_booster_config["expireBlock"]


def test_bond_booster_deployment_with_invalid_initial_boost_fails(bond_booster_fresh):
    """Test BondBooster deployment fails with invalid initial boost"""
    # Invalid boost with zero address
    invalid_boosts = [
        (
            ZERO_ADDRESS,
    ...
    with boa.reverts("invalid booster"):
        bond_booster_fresh(invalid_boosts)
