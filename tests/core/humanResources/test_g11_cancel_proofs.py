...
def test_g11_official_delta_cancel_pre_cliff_after_cash_refunds_full_and_burns(
    ...
    assert c.unlockTime() > c.cliffTime() or c.cliffTime() == c.unlockTime()
...
def test_g11_residue_b_plus_p_burns_only_comp_refunds(
    ...
    assert human_resources.hrGrant(c.address).settled
    assert human_resources.hrGrant(c.address).mintedPaycheck == p
...
def _grant_snapshot(
    ...
    g = human_resources.hrGrant(contributor.address)
    snap = {
        "grant": g,
...
def test_g11_pre_cliff_cash_then_unfrozen_after_cliff_cancel_settles(
    human_resources,
    setupHrConfig,
    setupLedgerBalance,
    setupRipeGovVaultConfig,
...
def test_g11_frozen_after_pre_cliff_cash_settles_c_minus_p_c_minus_p_c(
    human_resources,
    setupHrConfig,
    setupLedgerBalance,
    setupRipeGovVaultConfig,
    ...
    reserved = ledger.hrReservedCompensation()
    liability = ledger.hrCancelCreditLiability()
    assert reserved == orig - p
    assert liability == orig
    pos = ripe_gov_vault.getTotalAmountForUser(c, ripe_token)
    assert pos == p
    _, ok = official_delta_cancel(switchboard_delta, governance, c)
    assert ok is True
...
def test_g11_pre_cliff_cancel_burn_shortfall_rolls_back_then_retry(
    contributor_contract,
    setupRipeGovVaultConfig,
    switchboard_delta,
    governance,
    ...
    with pytest.raises(BoaError) as exc:
        switchboard_delta.executePendingAction(aid, sender=governance.address)
    assert_reverted_call(exc.value, "hr reserve underflow", switchboard_delta)
    assert switchboard_delta.hasPendingAction(aid)
    assert switchboard_delta.pendingCancelPaycheck(aid) == c.address
    assert switchboard_delta.getActionConfirmationBlock(aid) == snap["confirm_block"]
    assert _grant_snapshot(
        human_resources, ledger, c, ripe_gov_vault, ripe_token, switchboard_delta, aid
    ) == snap
    seed_ripe_gov_position(ripe_gov_vault, ripe_token, whale, teller, c.address, p)
    budget = ledger.ripeAvailForHr()
    orig = c.compensation()
...
def test_g11_pre_cliff_cancel_burn_reverts_rolls_back(
    contributor_contract,
    setupRipeGovVaultConfig,
    switchboard_delta,
    governance,
    ...
    original = _swap_ripe_runtime(
        ripe_token, "tests/core/humanResources/Group11BurnRevertRipe.vy"
    )
    try:
        with pytest.raises(BoaError) as exc:
            switchboard_delta.executePendingAction(aid, sender=governance.address)
        assert_reverted_call(exc.value, "burn mutant revert", switchboard_delta)
    finally:
        _restore_ripe_runtime(ripe_token, original)
    assert _grant_snapshot(
        human_resources, ledger, c, ripe_gov_vault, ripe_token, switchboard_delta, aid
...
def test_g11_ledger_registered_without_grant_cannot_cash_or_refund(
    human_resources,
    setupLedgerBalance,
    ledger,
    ripe_token,
    ...
    ghost = boa.env.generate_address("uninitialized-grant")
    ledger.addHrContributor(ghost, compensation, sender=human_resources.address)
    g = human_resources.hrGrant(ghost)
    assert g.initialized is False
    ...
    snap = {
        ...
        "liability": ledger.hrCancelCreditLiability(),
        "budget": ledger.ripeAvailForHr(),
        "vault": ripe_gov_vault.getTotalAmountForUser(ghost, ripe_token),
        "supply": ripe_token.totalSupply(),
        "hr_ripe": ripe_token.balanceOf(human_resources),
    }
    with boa.reverts("hr grant not initialized"):
        human_resources.cashRipeCheck(1, 0, sender=ghost)
    with boa.reverts("hr grant not initialized"):
        human_resources.refundAfterCancelPaycheck(0, False, sender=ghost)
    assert human_resources.hrGrant(ghost) == snap["grant"]
    assert ledger.hrReservedCompensation() == snap["reserved"]
    assert ledger.hrCancelCreditLiability() == snap["liability"]
    assert ledger.ripeAvailForHr() == snap["budget"]
    assert ripe_gov_vault.getTotalAmountForUser(ghost, ripe_token) == snap["vault"]
    assert ripe_token.totalSupply() == snap["supply"]
    assert ripe_token.balanceOf(human_resources) == snap["hr_ripe"]


def test_g11_settled_grant_rejects_further_cash_and_refund(
    human_resources,
    setupHrConfig,
    setupLedgerBalance,
    setupRipeGovVaultConfig,
    ...
    assert g.initialized and g.settled
    snap = _grant_snapshot(human_resources, ledger, c, ripe_gov_vault, ripe_token)
    with boa.reverts("hr grant settled"):
        human_resources.cashRipeCheck(1, 0, sender=c.address)
    with boa.reverts("hr grant settled"):
        human_resources.refundAfterCancelPaycheck(0, False, sender=c.address)
    assert _grant_snapshot(human_resources, ledger, c, ripe_gov_vault, ripe_token) == snap
