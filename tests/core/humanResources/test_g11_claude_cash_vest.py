...
def test_g11c_hr_cash_ripe_check_uncapped_is_clone_impersonation_only(
    human_resources,
    mission_control,
    switchboard_delta,
    contributor_template,
    ...
    """Uncapped HR cash from a registered clone reverts and leaves reserve/budget unchanged."""
    setupRipeGovVaultConfig()
    c = make_contributor(
        human_resources, mission_control, switchboard_delta, contributor_template,
        ledger, governance, terms(),
    )
    # a non-contributor cannot reach it at all
    ...
    absurd = c.compensation() * 1000
    reserved = ledger.hrReservedCompensation()
    with boa.reverts("hr reserve underflow"):
        human_resources.cashRipeCheck(absurd, 0, sender=c.address)
    assert ripe_token.totalSupply() == supply
    assert position(ripe_gov_vault, c, ripe_token) == 0
