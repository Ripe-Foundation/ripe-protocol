...
def _as_c(addr):
    return type("Handle", (), {"address": addr})()
...
def test_g11_near_uint256_budget_overwrite_cancel_rolls_back_then_retry_after_correction(
    ...
    assert switchboard_delta.actionType(aid_max) != 0
    official_delta_budget(
        switchboard_delta, governance, MAX_UINT256 - ledger.hrCancelCreditLiability()
    )
    aid, ok = official_delta_cancel(switchboard_delta, governance, c)
    assert ok is True
    assert c.compensation() == 0
...
def test_g11_hr_layer_over_refund_reverts_and_leaves_reserve_and_budget(
    ...
    g = human_resources.hrGrant(c.address)
    ...
    with boa.reverts("hr reserve underflow"):
        human_resources.refundAfterCancelPaycheck(
            g.remainingMintable + 1, False, sender=c.address
        )
    ...
    assert human_resources.hrGrant(c.address) == g
    assert c.compensation() != 0
...
def test_g11_two_h_clone_headroom_cancel_pair(
    valid_contributor_terms,
    setupHrConfig,
    human_resources,
    governance,
    ...
    aid_a = initiate_contributor(human_resources, governance, a)
    boa.env.time_travel(blocks=human_resources.actionTimeLock())
    ...
    aid_b = initiate_contributor(human_resources, governance, b)
    boa.env.time_travel(blocks=human_resources.actionTimeLock())
    ...
    assert ledger.hrReservedCompensation() == MAX_UINT256 - 1
    assert ledger.hrCancelCreditLiability() == MAX_UINT256 - 1
    ...
    official_delta_budget(switchboard_delta, governance, 1)
    official_delta_budget(switchboard_delta, governance, 0)

    _, ok_a = official_delta_cancel(switchboard_delta, governance, Contributor.at(addr_a))
    _, ok_b = official_delta_cancel(switchboard_delta, governance, Contributor.at(addr_b))
    assert ok_a is True and ok_b is True
    assert ledger.ripeAvailForHr() == MAX_UINT256 - 1
    assert ledger.hrReservedCompensation() == 0
    assert ledger.hrCancelCreditLiability() == 0
    official_delta_budget(switchboard_delta, governance, MAX_UINT256)
    assert ledger.ripeAvailForHr() == MAX_UINT256
...
def test_g11_third_h_clone_cannot_be_funded_but_c1_confirms(
    valid_contributor_terms,
    setupHrConfig,
    human_resources,
    governance,
    ...
    aid_h = switchboard_delta.setRipeAvailableForHr(h, sender=governance.address)
    travel_to_block(switchboard_delta.getActionConfirmationBlock(aid_h))
    with boa.reverts("exceeds hr budget headroom"):
        switchboard_delta.executePendingAction(aid_h, sender=governance.address)
    third = _valid(
        valid_contributor_terms,
        owner="0x" + "c5" * 20,
        manager="0x" + "c6" * 20,
    ...
    with boa.reverts("invalid terms"):
        initiate_contributor(human_resources, governance, third)
    assert human_resources.getTotalCompensation() == MAX_UINT256 - 1
    ...
    aid_one = initiate_contributor(human_resources, governance, one)
    boa.env.time_travel(blocks=human_resources.actionTimeLock())
    assert human_resources.confirmNewContributor(aid_one, sender=governance.address)
    addr_one = filter_logs(human_resources, "NewContributorConfirmed")[0].contributorAddr
    assert human_resources.getTotalCompensation() == MAX_UINT256
    assert ledger.hrReservedCompensation() == MAX_UINT256
    assert ledger.hrCancelCreditLiability() == MAX_UINT256
    for addr in (addr_a, addr_b, addr_one):
        _, ok = official_delta_cancel(switchboard_delta, governance, _as_c(addr))
        assert ok is True


def test_g11_full_cash_releases_reserved_then_max_budget_succeeds(
    human_resources,
    setupHrConfig,
    setupLedgerBalance,
    setupRipeGovVaultConfig,
    ...
    assert c.cashRipeCheck(sender=c.owner()) == c.compensation()
    assert human_resources.hrGrant(c.address).settled
    assert ledger.hrReservedCompensation() == 0
    assert ledger.hrCancelCreditLiability() == 0
    official_delta_budget(switchboard_delta, governance, MAX_UINT256)
...
def test_g11_after_cliff_partial_cash_cancel_releases_reserved(
    human_resources,
    setupHrConfig,
    setupLedgerBalance,
    setupRipeGovVaultConfig,
...
def test_g11_setter_after_pre_cliff_cash_uses_liability_not_mintable_gap(
    human_resources,
    setupHrConfig,
    setupLedgerBalance,
    setupRipeGovVaultConfig,
    ...
    """After pre-cliff cash P, setter headroom is still C, not C-P. Overlapping B stays safe."""
    setupRipeGovVaultConfig()
    settle_unsettled_hr_grants(human_resources, ledger)
    a_terms = _valid(
        valid_contributor_terms,
    ...
    assert p > 0
    assert ledger.hrReservedCompensation() == (c_amt - p) + b.compensation()
    assert ledger.hrCancelCreditLiability() == c_amt + b.compensation()
    ...
    aid_over = switchboard_delta.setRipeAvailableForHr(legal + 1, sender=governance.address)
    travel_to_block(switchboard_delta.getActionConfirmationBlock(aid_over))
    with boa.reverts("exceeds hr budget headroom"):
        switchboard_delta.executePendingAction(aid_over, sender=governance.address)

    official_delta_budget(switchboard_delta, governance, MAX_UINT256 - ledger.hrCancelCreditLiability())
    budget_pre_cancel = ledger.ripeAvailForHr()
    _, ok = official_delta_cancel(switchboard_delta, governance, a)
    assert ok is True
    assert a.compensation() == 0
    assert human_resources.hrGrant(a.address).settled
    assert ledger.ripeAvailForHr() == budget_pre_cancel + c_amt
    assert ledger.hrReservedCompensation() == b.compensation()
    assert ledger.hrCancelCreditLiability() == b.compensation()
    g_b = human_resources.hrGrant(b.address)
    assert g_b.initialized and not g_b.settled
    assert g_b.remainingMintable == b.compensation()


def test_g11_pre_cliff_cash_then_rest_at_end_settles_and_max_write_succeeds(
    human_resources,
    setupHrConfig,
    setupLedgerBalance,
    setupRipeGovVaultConfig,
    ...
    assert p > 0
    assert not human_resources.hrGrant(c.address).settled
    travel_to_ts(c.endTime())
    rest = c.cashRipeCheck(sender=c.owner())
    assert rest == c.compensation() - p
    assert c.totalClaimed() == c.compensation()
    assert human_resources.hrGrant(c.address).settled
    assert ledger.hrReservedCompensation() == 0
    assert ledger.hrCancelCreditLiability() == 0
    official_delta_budget(switchboard_delta, governance, MAX_UINT256)
