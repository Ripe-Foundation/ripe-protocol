...
def test_ah_auction_mgmt_only_mission_control_access(
    ...
    auction_house.startAuction(bob, vault_id, alpha_token, sender=switchboard.address)
    auction_house.startManyAuctions(auctions, sender=switchboard.address)
    auction_house.pauseAuction(bob, vault_id, alpha_token, sender=switchboard.address)
    auction_house.pauseManyAuctions(auctions, sender=switchboard.address)
...
def test_ah_auction_mgmt_paused_contract(
    ...
    auction_house.pause(True, sender=switchboard.address)
...
def test_ah_start_auction_success(
    ...
    result = auction_house.startAuction(bob, vault_id, alpha_token, sender=switchboard.address)
...
def test_ah_start_auction_invalid_conditions(
    ...
    result = auction_house.startAuction(bob, invalid_vault_id, alpha_token, sender=switchboard.address)
    assert result == False
    ...
    result = auction_house.startAuction(alice, vault_id, alpha_token, sender=switchboard.address)
    assert result == False
...
def test_ah_start_many_auctions_success(
    ...
    num_paused = auction_house.pauseManyAuctions(auctions_to_pause, sender=switchboard.address)
    assert num_paused == 4
    ...
    num_started = auction_house.startManyAuctions(auctions_to_start, sender=switchboard.address)
...
def test_ah_start_many_auctions_empty_list(
    ...
    num_started = auction_house.startManyAuctions([], sender=switchboard.address)
...
def test_ah_pause_auction_success(
    ...
    # Pause the auction
    result = auction_house.pauseAuction(bob, vault_id, alpha_token, sender=switchboard.address)
...
def test_ah_pause_auction_invalid_conditions(
    ...
    invalid_vault_id = 999999
    result = auction_house.pauseAuction(bob, invalid_vault_id, alpha_token, sender=switchboard.address)
    assert result == False
...
def test_ah_pause_many_auctions_empty_list(
    ...
    # Empty list should return 0
    num_paused = auction_house.pauseManyAuctions([], sender=switchboard.address)
    assert num_paused == 0
