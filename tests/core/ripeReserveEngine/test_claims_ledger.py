...
from constants import RIPE_RESERVE_ENGINE_HQ_ID, MAX_UINT256, ZERO_ADDRESS
from tests.core.ripeReserveEngine.conftest import travel_blocks
...
def test_position_creation_validates_engine_user_allocation_durations_and_overflow(
    lane_env, alice
):
    claims = lane_env.claims
    with boa.reverts("invalid engine"):
        claims.createVestingPosition(alice, 1, 1, 1, sender=alice)
    ...
    with boa.reverts("invalid allocation"):
        claims.createVestingPosition(
            alice,
            0,
            1,
    ...
    with boa.reverts("invalid minimum vesting"):
        claims.createVestingPosition(
            alice,
            1,
            1,
    ...
    with boa.reverts("invalid vesting length"):
        claims.createVestingPosition(
            alice,
            1,
            9,
    ...
    with boa.reverts("claim start overflow"):
        claims.createVestingPosition(
            alice,
            1,
            MAX_UINT256,
...
def test_create_position_stores_exact_terms_and_decrements_budget(lane_env):
    ...
    assert lane_env.claims.remainingAllocationBudget() == budget_before - allocation
    assert lane_env.claims.totalAllocatedRipe() == allocation
    assert lane_env.claims.totalClaimedRipe() == 0
    assert lane_env.claims.totalOutstandingRipe() == allocation
    assert event.user == lane_env.bob
    assert event.positionId == position_id
    assert event.sourceEngine == lane_env.lane.address
    assert event.ripeAllocation == allocation
    assert event.creationBlock == creation_block
    assert event.claimStartBlock == creation_block + minimum
...
def test_linear_vesting_boundaries_and_floor_rounding_after_minimum_cliff(lane_env):
    allocation = 101
    position_id = create_position(lane_env, allocation, 10)
    ...
    travel_blocks(1)
    assert lane_env.claims.getVestedRipe(lane_env.bob, position_id) == allocation
    assert lane_env.claims.getClaimableRipe(lane_env.bob, position_id) == allocation


def test_catch_up_cliff_releases_accrual_from_creation(lane_env):
    position_id = create_position(
        lane_env,
        1_000,
        10,
    ...
    position = lane_env.claims.positions(lane_env.bob, 1)
    assert position.claimStartBlock == position.creationBlock + 4
    assert position.maturityBlock == position.creationBlock + 10
    assert lane_env.claims.getVestedRipe(lane_env.bob, position_id) == 0
    travel_blocks(3)
    assert lane_env.claims.getVestedRipe(lane_env.bob, position_id) == 0
    assert lane_env.claims.getClaimableRipe(lane_env.bob, position_id) == 0
    travel_blocks(1)
    assert lane_env.claims.getVestedRipe(lane_env.bob, position_id) == 400
    assert lane_env.claims.getClaimableRipe(lane_env.bob, position_id) == 400
    travel_blocks(3)
    assert lane_env.claims.getVestedRipe(lane_env.bob, position_id) == 700


def test_equal_minimum_and_selected_duration_unlocks_fully_at_cliff(lane_env):
    position_id = create_position(
        lane_env,
        99,
        5,
        min_vesting_length=5,
    )
    travel_blocks(4)
    assert lane_env.claims.getVestedRipe(lane_env.bob, position_id) == 0
    travel_blocks(1)
    assert lane_env.claims.getVestedRipe(lane_env.bob, position_id) == 99
    assert lane_env.claims.getClaimableRipe(lane_env.bob, position_id) == 99
...
def test_partial_claim_updates_position_totals_and_event_without_refilling_budget(
    ...
    assert event.ripeAllocation == payout
...
def test_full_claim_removes_position_and_preserves_monotonic_totals(lane_env):
    ...
    assert tuple(lane_env.claims.positions(lane_env.bob, 1)) == (0, 0, 0, 0, 0, 0)
    assert lane_env.claims.getVestedRipe(lane_env.bob, position_id) == 0
    assert lane_env.claims.getClaimableRipe(lane_env.bob, position_id) == 0
...
def test_swap_and_pop_compacts_positions_and_reuses_tail_index(lane_env):
    ...
    assert tuple(lane_env.claims.positions(lane_env.bob, 3)) == (0, 0, 0, 0, 0, 0)
...
def test_vesting_authorizes_the_live_hq_engine(lane_env):
