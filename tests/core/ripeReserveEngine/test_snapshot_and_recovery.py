...
def transferFrom(_from: address, _to: address, _value: uint256) -> bool:
    ...
    self.allowance[_from][msg.sender] -= _value
    self.balanceOf[_from] -= _value
    self.balanceOf[_to] += _value + convert(self.mode == 2, uint256)
    return True
"""
...
def test_no_return_payment_token_is_supported(lane_factory, governance):
    token = boa.loads(NO_RETURN_PAYMENT_TOKEN, governance.address, 6)
    scale = 10**6
    ctx = lane_factory(payment_token=token, fund_buyer=False)
    token.transfer(ctx.bob, scale, sender=governance.address)
    token.approve(ctx.lane, MAX_UINT256, sender=ctx.bob)

    allocation = ctx.buy(scale)

    assert allocation > 0
    assert token.balanceOf(ctx.endaoment_funds) == scale
    assert ctx.claims.totalAllocatedRipe() == allocation


@pytest.mark.parametrize(
    ("mode", "expected_error"),
    (
        pytest.param(1, "payment failed", id="false-return"),
        pytest.param(2, "payment receipt mismatch", id="excess-receipt"),
    ),
)
def test_invalid_payment_receipts_revert_all_acquisition_state(
    lane_factory,
    governance,
    mode,
    expected_error,
):
    token = boa.loads(ADVERSARIAL_RECEIPT_TOKEN, governance.address, 6)
    scale = 10**6
    ctx = lane_factory(payment_token=token, fund_buyer=False)
    token.transfer(ctx.bob, scale, sender=governance.address)
    token.approve(ctx.lane, MAX_UINT256, sender=ctx.bob)
    ...
    budget_before = ctx.claims.remainingAllocationBudget()
    buyer_before = token.balanceOf(ctx.bob)
    funds_before = token.balanceOf(ctx.endaoment_funds)

    with boa.reverts(expected_error):
        ctx.buy(scale)
    ...
    assert ctx.claims.nextPositionId() == 1
    assert ctx.claims.remainingAllocationBudget() == budget_before
    assert token.balanceOf(ctx.bob) == buyer_before
    assert token.balanceOf(ctx.endaoment_funds) == funds_before
