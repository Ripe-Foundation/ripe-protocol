...
from tests.core.ripeReserveEngine.conftest import config_dict, make_config
...
def test_runtime_sizes_have_eip170_headroom(lane_env, ripe_hq):
    ...
    report_path = os.environ.get("RIPE_RESERVE_ENGINE_SIZE_REPORT")
    if report_path:
...
def test_purchase_and_claim_gas_benchmarks_are_reported(lane_env):
    ...
    report_path = os.environ.get("RIPE_RESERVE_ENGINE_GAS_REPORT")
    if report_path:
