...
def test_direct_claim_mints_only_the_newly_vested_amount(lane_env):
    ...
    assert lane_event.requestedLockDuration == 0
    ...
    assert (
        filter_logs(lane_env.lane, "VestedRipeClaimed")[-1].requestedLockDuration
        == 999
    )
...
def test_auto_deposit_claim_mints_to_lane_then_deposits_exactly(lane_env):
    ...
    assert event.requestedLockDuration == 500
...
@pytest.mark.parametrize(
    ("requested_lock", "applied_lock"),
    (
        pytest.param(0, 100, id="minimum-clamp"),
        pytest.param(2_000, 1_000, id="maximum-clamp"),
    ),
)
def test_auto_deposit_event_reports_requested_not_applied_lock(
    lane_env,
    requested_lock,
    applied_lock,
):
    ...
    position_id, _ = _purchase(lane_env, 1)
    boa.env.time_travel(blocks=1)
    claim_block = boa.env.evm.patch.block_number

    lane_env.claim(
        position_id,
        auto_deposit=True,
        lock_duration=requested_lock,
    )

    event = filter_logs(lane_env.lane, "VestedRipeClaimed")[-1]
    assert event.requestedLockDuration == requested_lock
    user_data = lane_env.ripe_gov_vault.userGovData(
        lane_env.bob,
        lane_env.ripe_token,
    )
    assert user_data.unlock == claim_block + applied_lock
