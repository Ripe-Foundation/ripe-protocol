...
TELLER_LABEL = "TellerPriceDeskGasCandidate20260919"
SOURCE_NAMES = (
...
def migrate(migration: Migration):
    ...
    log.h1("4. Stage the compatible Teller, paused")
    teller = migration.deploy(
        "Teller", hq.address, True, params["CURVE_PRICES_ID"], label=TELLER_LABEL,
    )
    ...
    if str(hq.getAddr(17)).lower() != str(old_teller).lower():
        raise RuntimeError("BASE_GAS_ACTIVE_TELLER_CHANGED")
    log.info(f"STAGED BRIDGE PRICEDESK (HQ slot 7 candidate): {desk.address}")
    log.info(f"STAGED PAUSED TELLER (HQ slot 17 candidate): {teller.address}")
