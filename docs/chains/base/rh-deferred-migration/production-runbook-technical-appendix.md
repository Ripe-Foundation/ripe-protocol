...
catalogs, edge cases, source anchors, and execution-record templates supporting
the concise [production runbook](./production-runbook.md).

**Status:** technical appendix; **not execution-ready and not authorization**

Read the concise production runbook first. This appendix preserves the detailed
engineering and verification requirements removed from its primary flow. If
the two documents appear inconsistent, stop and reconcile them before payload
authorship; neither document authorizes choosing the more convenient rule.
