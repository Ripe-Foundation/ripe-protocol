...
Authenticated historical fixture whitespace is intentional and must remain byte-for-byte unchanged. Check this round normally; for the complete stacked diff, narrowly exclude only `tests/fixtures/legacy_pool/contracts/vaults/StabilityPool.vy`, `tests/fixtures/legacy_pool/contracts/vaults/modules/StabVault.vy` and `tests/fixtures/legacy_pool/contracts/vaults/RipeGov.vy` if their authenticated source bytes trigger `git diff --check`. Verify provenance hashes instead; do not normalize those files.
