...
- `RebaseErc20` with `SharesVault` immediately reflects the live token balance,
  ...
  atomically. At total loss, live collateral value and borrowing capacity
  become zero while the non-empty zero-amount position and its configured
  resolution/debt terms remain represented; liquidation eligibility is
  retained and an auction can start. Auction settlement remains
  vault-dependent and currently fails atomically with the established
  `no asset to withdraw` behavior, and the tested Deleverage withdrawal still
  cannot progress. This establishes no restoration, settlement safety,
  activation, launch readiness, or complete bad-debt resolution; M2, composed
  M4 evidence, and M5 remain mandatory. Fresh deposits after zero also dilute
  the old shares to a zero-rounded claim.
...
- `CreditEngine._getUserBorrowTerms` iterates
  `getUserAssetAndAmountAtIndex` and prices the returned common amount when it
  is non-zero (`CreditEngine.vy:687-807` at `4c26d7d`). Simple therefore
  retains borrowing power against missing assets. Rebase immediately lowers
  borrowing power with live custody. At zero, M3 excludes only the
  empty-address sentinel: the non-empty zero-amount position remains
  represented, is assigned zero collateral value and zero borrowing capacity
  without consulting PriceDesk for the zero amount, and keeps its configured
  resolution terms represented under the existing minimum-weight fallback, so
  liquidation eligibility is retained. This does not make settlement safe and
  does not close M2, composed M4 evidence, or M5.
...
claim. Rebase also retains liquidation eligibility: the non-empty zero-amount
position remains represented with zero live collateral value and zero
borrowing capacity while its configured resolution terms stay represented, so
liquidation can begin and an auction can be created. The unresolved Rebase
failure is vault-dependent settlement, not liquidation or auction creation:
the purchase attempt currently fails atomically with `no asset to withdraw`,
leaving GREEN, debt, buyer collateral, token balances, and the open auction
unchanged. This establishes no restoration, safe settlement, activation,
readiness, or complete bad-debt resolution; M2, composed M4 evidence, and M5
remain mandatory.
...
Every test reference in the table cites final test evidence commit `05940a5`,
which contains the complete 90-case suite and matches the file hash recorded
above. Test-introduction history remains available in Git but is not mixed into
the final evidence citations.
...
| A-07 | Issuer burns all before liquidation starts | Total burn before auction | Missing collateral cannot create paid auction | Starts auction; 20 GREEN buys 40 zero-backed nominal | Zero-backing position remains represented; liquidation eligibility retained; auction starts; settlement remains vault-dependent | 0 | Nominal/shares remain | 200 vs 0 before purchase | Simple phantom threshold; Rebase zero-backing terms remain represented | Simple buyer cannot withdraw | Simple unsafe settlement | Rebase external purchase reverts atomically (`no asset to withdraw`); GREEN and debt unchanged | None | Both Conform | Simple **Unsafe**; Rebase settlement remains blocked — no restoration, settlement safety, activation, or readiness established | **Not accepted** | Owner approved analysis continuation only | `test_auction_started_after_total_issuer_loss` @ `05940a5`, updated @ `4c26d7d` | **Tested:** both vaults; Rebase auction starts and external settlement fails atomically with the established `no asset to withdraw` behavior, leaving GREEN, debt, and the open auction unchanged; M2, composed M4 evidence, and M5 remain mandatory | Shared total-loss specification required |
| DV-01 | Governance volatile-asset Deleverage after partial then total loss | Partial then total burn | Repaid debt equals delivered collateral; zero failure atomic | Partial repays 20; zero attempt reverts | Same | 80 after partial; 0 after total | Reduced on partial; unchanged on failed zero attempt | Live/nominal semantics persist | Debt -20 only on delivery | External delivery only | N/A | Atomic | Rebase rounding none at scale | Both Conform | Safe atomicity; total progress blocked | Stock Tokens must not route to Endaoment in production | Pending | `test_deleverage_external_withdrawal_after_partial_and_total_loss` @ `05940a5` | **Tested:** applicable external path; **Source-traced:** Deleverage has no internal mode | Keep integrations disabled; specify zero path |
...
- Rebase pro-rata partial-loss treatment: recommended behavior, not yet owner
  approved;
- Rebase donation allocation, raw-share rewards, post-zero dilution, and the
  total-loss vault-dependent settlement dead end that remains after successful
  auction creation: **not accepted**;
