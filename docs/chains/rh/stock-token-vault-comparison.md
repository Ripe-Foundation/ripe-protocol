...
**Final test evidence commit:** `ee270abd317daae25d434a4a256346b5a0cb95d3`
...
| `tests/vaults/test_stock_token_vault_comparison.py` | `5c4798dacc251f0d40e432938aaf11850745674e4d1803ce6bf36fe0c4a34d1a` |
...
- `Lootbox` uses `getUserLootBoxShare`, then divides by token precision for
  ...
  Simple, live for Rebase. Monitoring must not label Rebase raw shares as tokens.
...
6. Observe: Alice spends 20 GREEN; Bob's debt falls by 20 GREEN; the internal
   ...
   collateral value to Alice; Alice's withdrawal reverts.
...
The original 60 cases use evidence commit `d8f11e9`; the final six cases use
`ee270ab`, which contains the complete 66-case suite.
...
| I-04 | Block vault sender, user recipient, or vault operator before withdrawal | Blocklist | Role-specific atomic failure | All three fail/retry | Same | Unchanged after failure | No change | Preserved | Preserved | Fails then succeeds | No token role check internally | Fails externally | None | Both Conform | Safe atomicity; internal caveat | Accepted architecture input | Pending | `test_withdraw_blocklist_roles_and_retry` @ `d8f11e9` | Exact ERC-20 role, not address label, determines result | Monitoring required |
...
1. Track 2 exact token identity, decimals, current implementation, and live
   third-party-contract transferability.
