...
| CM-051 | GREEN CCIP BurnMint pool | Net-new shared pure-Vyper integration; v1.6.1-shaped reference exists | N/A / counterpart required on Base | Burn/mint GREEN across Base and RH | deferred | Yes, same integration source both chains | Production specification, Chainlink eligibility, audit, gas, and activation remain pending Track 1 |
| CM-052 | RIPE CCIP BurnMint pool | Net-new shared pure-Vyper integration derived from the same reviewed behavior | N/A / counterpart required on Base | Burn/mint RIPE across Base and RH | deferred | Yes, same integration source both chains | Same production gates as CM-051; opposite Ripe mint capability |
...
| CM-057 | ABI export and explorer verification | `scripts/export_abis.py`, `scripts/verify.py`, `scripts/utils/verify_etherscan.py` | Vyper ABI export and Etherscan/Basescan verification | Verify core and CCIP Vyper artifacts on RH explorer | modified | Shared tooling | Export already scans Vyper; verification still assumes Etherscan-style/Base paths and must preserve explicit EVM/compiler metadata |
| CM-058 | CCIP Vyper parity/build/test boundary | Existing Vyper/titanoboa workflow; production pool not yet implemented | Not required by current deployed core | Build, differentially test, deploy, and verify pure-Vyper CCIP pools | deferred | Yes | Exact Chainlink oracle pin, EVM target, ABI bounds, audit, gas, and CI ownership remain gated |
...
| CM-051–052 | No existing deployment; Base counterpart pools are required if approved | Proposed RH pools; only GREEN/RIPE bridged | Chainlink CCIP Router, RMN proxy, Token Admin Registry; no protocol block clock assumed | Deferred net-new shared pure-Vyper source; once frozen, identical pool implementations deploy on Base/RH while core Vyper remains common | Pending Track 1 custom-pool eligibility/version. Specify owner/admin, standard lifecycle/rate limits/events/errors, Vyper bounds, RipeHq capability, gas and emergency controls; differential/unit/integration/fork tests and independent audit | Executive summary; external facts pending Track 1 |
...
| CM-057 | Vyper ABI exporter and Etherscan/Basescan assumptions | Export all Vyper ABIs, including CCIP; verify RH explorer using supported API | Explorer and compiler metadata | Shared tooling modified | Tool/EVM version pinning, reproducible compiler inputs, verification smoke tests | Scripts/ABIs; high |
| CM-058 | Current production build is Vyper-centric | Extend the same pinned Vyper/titanoboa path for CCIP parity, fuzz, fork and gas tests | Exact Chainlink source/API as differential oracle, not a build dependency | No new language boundary or second deployment truth source | Lock Vyper/EVM/bounds/oracle versions and CI; selector/event/error/lifecycle parity plus audit and integration tests | Repository/tooling audit and Track 1; high on gap |
...
| CCIP pure-Vyper parity boundary | Existing pinned Vyper/titanoboa; untracked alternate build | Owner selected pure Vyper; the implementation must reproduce the accepted Chainlink behavior without inherited audit coverage | Use one repo-native Vyper build/deploy truth source; pin EVM target, ABI bounds and Chainlink differential oracle; add parity/fork/gas/audit gates | CM-051–053, CM-057–059 | Engineering/deployment/security owner | Before production CCIP code | Direction selected; exact gates remain open |
...
- “Pin the supported CCIP pool/API reference and decide how the selected
  ...
  verified, and deployed with an explicit EVM target and ABI bounds.” (reference
  facts pending Track 1; language/toolchain direction selected).
