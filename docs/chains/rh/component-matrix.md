...
| CM-025 | `RebaseErc20` / inherited `SharesVault` | `contracts/vaults/RebaseErc20.vy`, `contracts/vaults/modules/SharesVault.vy` | Share-accounted collateral option | Preferred accounting direction, but not approved unchanged | deferred | Yes | Track 5 proves pro-rata partial-loss accounting but leaves total-loss progress, post-zero allocation, deposit measurement, rewards, and monitoring unresolved |
| CM-026 | `AuctionHouse` | `contracts/core/AuctionHouse.vy` | Liquidation auctions | Local liquidation auctions; Stock Token settlement policy pending | reused unchanged | Yes | Track 5 requires the follow-on specification to disposition live-backing and external-only settlement; reclassify if shared source changes are approved |
...
| CM-030 | `CreditEngine` | `contracts/core/CreditEngine.vy` | Borrowing, debt and interest | Local GREEN credit engine; Stock Token deficit policy pending | reused unchanged | Yes | Track 5 requires the follow-on specification to disposition fail-closed valuation and total-loss debt health; reclassify if shared source changes are approved |
...
| CM-024–025 | Both implementations exist in Base manifest | No Stock Token vault is approved unchanged; corrected share accounting is the preferred direction | Exact AAPL identity and fork probe are integrated; live proof and exact-token vault integration remain open | Any approved change must be shared; selection is not a fork | Specify deposit delta, live-backed settlement, total-loss debt progress, post-zero allocation, rounding, rewards, registry/migration, and the exact-token test matrix | Track 5 tests/decision + Track 2 evidence; high on current behavior, owner/specification open |
| CM-026–027 | Base auction delay/duration and local dependencies | RH auction values and roles; Stock Token settlement remains disabled until specified | MissionControl, CreditEngine, vaults; BN-030/031 | Current source unchanged; follow-on vault specification may require a shared change and live-version plan | Clock tests plus Track 5 live-backing, external-only settlement, atomic payment, and zero-custody progress invariants | Source/tests + Track 5; high on current behavior, design open |
...
| CM-030 | Borrow interval `43,200`; seconds interest; Curve danger boost consumes CAD-001 | Provisionally `7,200` for one day; seconds interest unchanged; base-rate fallback while Curve is absent; Stock Token borrowing disabled until deficit behavior is approved | Oracles/vaults; BN-029, TS-010, CAD-001 | Current source unchanged; follow-on vault specification may require shared fail-closed valuation and deficit-aware debt-health changes | Debt/rate and clock tests plus Track 5 zero-custody borrowing, liquidation eligibility, and bad-debt invariants | Source/defaults/tests + Track 5; high on current behavior, design open |
...
| Stock Token vault | `SimpleErc20`; corrected `RebaseErc20`/`SharesVault`; do not list | Track 2 identifies AAPL and passes a pinned-fork probe; Track 5 rejects both vaults unchanged and proves the Simple zero-backed internal-auction failure | Open an owner-approved shared vault-change specification using corrected share accounting as the preferred direction; do not enable Stock Token collateral before its invariants, exact-token tests, and live gate close | CM-021, CM-024–026, CM-030, CM-043 | Protocol/risk/security owner | Before asset migration or borrowing enablement | Blocked pending owner-approved specification and Track 2 live gate |
