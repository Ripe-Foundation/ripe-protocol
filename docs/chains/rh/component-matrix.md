...
**Status:** Complete analysis; S3, S5, and Stock Token launch-scope decisions
are recorded; remaining recommendations and owner decisions are unapproved
...
**Minimum-change revision:** 2026-07-24 — CM-014 and CM-044 remain behind the
S4 necessity gate. The owner retained CM-033 S3, selected CM-008's portable
...
| CM-014 | `SwitchboardDelta` | `contracts/config/SwitchboardDelta.vy` | Deleverage, bond and operational configuration | Same role; initial candidate keeps Deleverage cooldown zero | reused unchanged | Yes | Duplicate cooldown cap is dormant at zero; any nonzero policy or separate HR-bound issue requires a later necessity decision |
...
| CM-044 | `Deleverage` | `contracts/core/Deleverage.vy` | Withdrawal deleveraging and cooldown | Same role with cooldown disabled at zero | reused unchanged | Yes | Portable nonzero cooldown is deferred unless owner rejects no-pacing risk |
...
| CM-014 | `Deleverage.deleverageCooldown` storage initializes to `0`; no repository snapshot proves current live state; duplicated max `7,200`; Base bond terms | Initial candidate: keep cooldown zero and existing validation; separately configure other RH terms | MissionControl, Deleverage, BondRoom/BondBooster, Ledger, HumanResources/Contributor; BN-002/012/013/016/017/032 + TS-002 | No Deleverage/Delta bytecode divergence if zero cooldown is accepted | Explain lost pacing protection; prohibit nonzero activation until a later spec; treat any independent HR-bound defect separately | Source/tests; high mechanics |
...
| CM-044 | Contract-local cooldown storage initializes to `0`; current live value is not in repository snapshots; max duplicated at `7,200` | Initial candidate keeps `0` and existing source | SwitchboardDelta/Teller; BN-012 | No source divergence; nonzero enablement remains gated | Quantify zero-cooldown risk; implement context/max only after owner necessity approval | Source/tests; high mechanics |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Live Base parity | Reuse unchanged; upgrade Base first/together; allow time-bounded divergence; accept justified permanent exception | CM-008 is a state-bearing live Ledger whose migration is unacceptably risky; CM-014/044 retain a no-change candidate; CM-033 S3 is retained | Accept permanent live-bytecode divergence for CM-008 while preserving one forward canonical source; keep S4 unchanged unless separately approved; treat S3 convergence as a separate live rollout | CM-008, CM-014, CM-033, CM-044; future CM-051/052 counterparts | Protocol/deployment owner | Release freeze | CM-008 permanent exception and S3 direction owner-approved; S4 open |
| Shared clock posture | Retain/configure/disable; convert all; redesign semantic failures | Inventory maps 100 literals plus CAD-001; BN-002 is a true execution-block identity requirement, BN-012 is dormant at zero, and BN-025 is implemented narrowly | Retain/configure first; implement the approved narrow BN-002/025 changes; require a separate necessity decision for BN-012 | CM-008, CM-009, CM-011–014, CM-017, CM-029–030, CM-033, CM-038, CM-044, CM-049, CM-059 | Protocol/security owner | Block-clock specification | Analysis and BN-002/025 directions approved; remaining values/changes open |
| Cooldown maximum intent | Keep cooldown zero; preserve Base numeric `7,200`; preserve one-day intent; another bound | Duplicate cap is dormant while cooldown is zero | Accept zero/no pacing for launch unless owner requires nonzero protection | CM-014, CM-044 | Security/protocol owner | S4 checkpoint | Open |
