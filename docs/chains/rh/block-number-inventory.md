...
| CCIP pure-Vyper parity boundary | Existing pinned Vyper/titanoboa path; external untracked build process | Owner selected pure Vyper; CCIP compatibility now depends on explicit parity, EVM-target, ABI-bound, gas, and audit evidence | Use the existing repo-native build/deploy truth source and avoid a parallel untracked process | CM-051–053, CM-057–059 | Engineering/deployment/security owner | Before production CCIP code | Direction selected; exact gates open |
