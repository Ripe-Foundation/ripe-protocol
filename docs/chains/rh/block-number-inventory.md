...
- **Source disposition:** `retain` means the shared runtime expression can remain;
  ...
  `modify shared` means the canonical source needs a chain-portable change.
...
| BN-002 | `Ledger.checkAndUpdateLastTouch`: reject `lastTouch == block.number` | true same-number security guard; liquidation/borrow safety | Enabled in Base defaults; intended one higher-risk action per block | Repeat rejects later higher-risk transactions for the user, potentially for many L2 transactions; jump merely clears the guard | Robinhood L1-derived numbering can turn a same-block MEV guard into user throttling | **Modify shared or explicitly disable only after security approval.** Specify the protected threat and a chain-portable guard; do not silently set false | Security/protocol owner; high on mechanics, policy unresolved |
...
| BN-012 | `Deleverage.deleverageForWithdrawal`: cooldown comparison and last block | hardcoded economic duration; security/rate limit | Contract-local `deleverageCooldown` storage initializes to `0`; no default, migration assignment, or generated snapshot value exists. It is hard-capped by duplicated `7,200`; comments call the cap “~1 day at 12s/block,” but on Base it is about 4 hours | Deliberate `block.number > lastBlock` allows same-number multi-asset work; on RH it also permits many repeated-number transactions. A jump consumes cooldown | Repeated-number bypass; duplicated cap; ambiguous 4h-vs-1d intent | **Modify shared.** Centralize/parameterize the hard max and specify a call-context-safe exception instead of relying on same-number identity. Owner must choose 4h (`7,200` Base / `1,200` RH) or 1d (`43,200` Base / `7,200` RH) intent | Security/protocol owner; high mechanics, intent unresolved |
...
| BN-025 | `Lootbox.distributeUnderscoreRewards`: send interval | hardcoded economic duration; unsupported integration | Governed interval, but constructor/setter enforce `ONE_DAY=43,200 # on Base`; Base value `43,200` | Repeat blocks sends; jump opens them | Shared source cannot accept a one-day RH value (`7,200`); embedded Base cadence remains even if disabled | **Modify shared** to parameterize the minimum. Configure RH path disabled/omitted; still remove the canonical Base-only hardcode | Protocol/rewards owner; high |
...
| BN-002 | The prior-number `lastTouch` no longer equals the current number, so the same-number rejection clears. |
...
| BN-002 | `lastTouch`; `Ledger.checkAndUpdateLastTouch`; caller flag derived by Teller from `MissionControl.shouldCheckLastTouch` and action risk; governed path `SwitchboardDelta.setShouldCheckLastTouch` → `MissionControl.setShouldCheckLastTouch` | `DefaultsBase.shouldCheckLastTouch=True`; `DefaultsLocal=False`; the 2025-12-02 `ledger_output.md` is runtime accounting evidence, not the flag's configuration source | `tests/data/test_ledger.py`, `tests/config/test_switchboard_delta.py`; add many-transactions/same-number and liquidation/borrow threat cases |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Live Base parity | Upgrade Base before/together with RH; allow time-bounded drift; accept a justified permanent exception | Pinned source has no production delta; only 2025-12-02 generated snapshots represent deployed values. Shared fixes would change Base bytecode if deployed | Permit temporary divergence only with an explicit compatibility review, owner, bound, and convergence plan; do not default to permanent divergence | BN-002, BN-012, BN-025; CM-008, CM-014, CM-033, CM-044 | Protocol/deployment owner | Implementation release plan | Open |
| Shared clock posture | Retain/configure acceptable block clocks; timestamp-convert all; redesign only semantic failures | BN-001–032 plus CAD-001 separate configurable clocks from Ledger/Deleverage/Lootbox failures | Retain economic block clocks where semantics survive; parameterize cadence assumptions; design shared replacements only for BN-002/012/025 | BN-001–032, CAD-001 | Protocol/security owner | Block-clock specification approval | Open |
| Cooldown maximum intent | Preserve numeric `7,200` (about 4h on Base); preserve comment intent (about 1d); choose another limit | Duplicated cap/comment and contract-local setter path are verified; no configured live value is evidenced by repository snapshots | State the intended wall time, centralize the limit, then derive chain values | BN-012; CM-014, CM-044 | Security/protocol owner | Deleverage specification | Open |
| Ledger same-number guard | Portable replacement; explicitly disable on RH; retain and accept throttling | BN-002 shows repeat-number rejection while `SwitchboardDelta` can govern the flag | Specify the protected threat and implement a shared portable guard; disable only with explicit security acceptance | BN-002; CM-008, CM-034 | Security owner | Ledger specification and RH config | Open |
...
| Stock Token vault | `SimpleErc20`; `RebaseErc20`/`SharesVault`; separately specified implementation | Source offers both paths; behavioral transferability and issuer-control evidence remains owned by Track 2 | Use Simple only if Track 2 and owner accept its failure behavior; otherwise specify SharesVault invariants | CM-021, CM-024, CM-025 | Protocol/risk owner | After Track 2, before asset migration | Pending Track 2 and owner |
...
| Solidity toolchain boundary | Foundry; another pinned repo-native toolchain; external untracked build process | Current repository build/export/verification is Vyper-centric while CCIP pools are Solidity | Commit one pinned repo-native build/test/deploy/verify path and avoid a parallel untracked process | CM-051–053, CM-057–059 | Engineering/deployment owner | Before CCIP code | Open |
...
3. design BN-002, BN-012, and BN-025 without `chain.id` branches;
