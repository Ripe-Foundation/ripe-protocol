...
**S3 planning correction:** 23 July 2026 — removed the nonexistent
...
**S4 decision closure:** 24 July 2026 — owner and independent-security review
closed S4 without production implementation for the initial release. Base and
Robinhood retain the existing shared Deleverage and SwitchboardDelta source;
Robinhood keeps `deleverageCooldown = 0` and omits Underscore. The dormant
maximum/context redesign is deferred and S4 must reopen before Underscore
inclusion or any nonzero cooldown proposal or queued action. See
...
These conversion rules and numeric candidates remain **recommended/open** unless
the decision register records narrower approval. The S3 Lootbox floor is the
only current exception: its `43_200` Base and `7_200` Robinhood values use this
cadence basis under the owner's dated, S3-only approval.
...
the exact boundary. The owner recorded protocol/rewards approval of the immutable
design, exact values, strict boundary, and Base convergence policy on 23 July
2026; implementation and live rollout remain open.
...
| S4 — Deleverage cooldown/context | BN-012; CM-044,014,034. Stage A decision record only for the initial release; no Stage B/C file set | Owner and independent security accepted zero cooldown, omitted Underscore, and prohibited future enablement without reopening | Existing Deleverage/Delta/Teller source remains unchanged; zero cooldown; no S4 production artifact or state-changing migration | Track 7 H-08 later proves live zero, actual Underscore omission, and absence of prohibited pending actions | **Closed no-code for initial launch.** Reopen before Underscore inclusion or any nonzero proposal/queue; `0020` is omitted or assertion-only and never state-changing |
...
Recommendations are not approvals. Only an explicit status and dated approval
provenance make a decision final; every other clock parameter remains open.
...
On 23 July 2026, the owner approved the four S3 decisions after independent
review of the S3 implementation brief: a nonzero/non-maximum immutable
per-deployment Lootbox floor and getter, preservation of strict `>` eligibility,
Base floor `43_200`, Robinhood floor `7_200`, and coordinated eventual Base
convergence with only bounded temporary live-bytecode drift and no permanent
divergence. The owner selected the approximately 2-second Base / 12-second
Robinhood cadence basis specifically for this isolated floor. That narrow
approval does not approve the cadence basis for S6 rates, timelocks, capacities,
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Shared clock posture | retain/configure; timestamp conversion; redesign all | Track 3 separates acceptable clocks from BN-002/012/025 defects; 24 July minimum-change directive and S4/S5 owner decisions | Retain observed-number semantics where valid; S3 and S5 are approved narrow shared directions; S4 is closed no-code with zero cooldown | all BN/CAD; CM-007–059 | Protocol/security | S3–S9 | analysis plus S3/S4/S5 directions owner-approved; remaining changes/values open |
...
| Deleverage Base version policy | unchanged source with zero cooldown; coordinated Deleverage/Delta/Teller upgrade; time-bounded drift; permanent divergence | zero cooldown keeps the current cap/exception dormant; if S4 later implements, its shared artifacts and ABI/context must move together | Base and RH keep unchanged source and no S4 rollout exists for the initial release. If S4 is reopened and implementation is approved, require one atomic reviewed Base rollout or a separately approved live-version policy; do not presume permanent divergence | BN-012; CM-014,034,044 | Security/protocol/deployment | S4 | owner/security-approved no-code launch posture; implementation contingency deferred |
